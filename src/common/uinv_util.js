...
/**
 * 弹警告框
 * @param {Array|String} s 数组或字符串
 * @modified by jw 2016.09.09
 * 数组例子：[No data is read under \"{1}\" when the combined animation \"{0}\" is played!,[[device_0_in_other_task,[\"test\"]],\"test2\"]]
 * 字符串中多个{n}时，比如saveTaskForStockAndRacking_0_error{n}[device_0_in_other_task{n}[\"test\"]]，则将后一个作为参数。切记test必须带引号
 * @param {Boolean} isNoAlert 如果为true，则不alert
 * @example uinv.util.alert("没有与此相关的设备数据！");
 * @return {promise} jquery的promise对象。可以用then/done来回调
 */
uinv.util.alert = function (s, isNoAlert) {
  return uinv.invokeMethodInBrowser('uinv.util.alert', [s, isNoAlert], function (s, isNoAlert) {
    var deferred = Q.defer(); //在函数内部，新建一个Deferred对象
    if (!isNoAlert) {
      var msg = uinv.util.getMsg(s) + '';
      if (typeof layer === 'undefined') {
        alert(msg);
        deferred.resolve();
      } else {
        var that = this;
        if (this._alertIndex) { //如果alert窗口已存在的话，将当前信息添加到后面
          var lastMsg = layer.getContent(this._alertIndex);
          if (lastMsg) {
            var lastArr = lastMsg.split('\n');
            if (lastArr.indexOf(msg) != -1) {
              return;
            }
            msg = lastMsg + '\n' + msg;
          }
        ...
        msg = msg.replaceAll('\t', '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;');//8个空格
        var config = {
          content: msg,
          title: u.le.get('DCV_MESSAGE'),
          btn: u.le.get('DCV_CONFIRM'),
          maxWidth: 400,
          maxHeight: 247.2,
          skin: 'alert',
          // btnAlign: 'c',//居中
        ...
        this._alertIndex = layer.open(config);
      }
    } else {
      deferred.resolve();
    }
 ...
 * @param {Array|String} s 数组或字符串
 * @return {Boolean}
 * @example uinv.util.confirm([ "注册模块{0}重复定义{1},是否是否需要断点到程序?", [ n, "" + name ] ]);
 */
uinv.util.confirm = function (s) {
  return confirm(this.getMsg(s));//jw 2016.11.22
};

/**
 * layerUi版本的confirm
 * @param {Array|String} s 数组或字符串
 * @return {Boolean|promise} 原生的返回true或false，非原生的返回jquery的promise对象，可以用then/done来链式处理，参数为true或false
 * @author jw
 * @date 2017-10-20
 */
uinv.util.confirm2 = function (s) {
  return uinv.invokeMethodInBrowser('uinv.util.confirm2', [s], function (s) {
    var msg = uinv.util.getMsg(s) + '';
    var deferred = Q.defer(); //在函数内部，新建一个Deferred对象
    //如果信息中存在换行符，再处理一下
    if (msg.indexOf('\n') != -1) {
    ...
    var config = {
      ...
      maxHeight: 247.2,
      skin: 'alert',
      btn: [u.le.get('DCV_CONFIRM'), u.le.get('DCV_CANCEL')],
      // btnAlign: 'c',//居中
      // shadeClose: false, //不能点击空白消失
      cancel: function () {
 ...
 * 更新token
 */
uinv.util.refreshToken = function () {
  console.log('-----------------token---------------------');
  // uinv.server.manager.admin.log("刷新token一次", "info");
  uinv.server.manager.admin.getRefreshedToken().then(function (token) {
    console.log('token:' + token);
    uinv.util.setToken(token);
  }, function (err) {
    // debugger;
    // uinv.server.manager.admin.log(err).catch(function (msg) {
    //   console.error("打印日志出错：" + msg);
    // });
    console.error(err);
  });
};
 ...
 * @date 2018-01-06
 */
uinv.util.setToken = function (token) {
  uinv.setToken(token);
  try {
    if (uinv.isUseNode) { //对应一端要更新
      if (uinv.isInNode()) { //在node端
        uinv.invokeBrowser({
          path: 'uinv.setToken',
          param: token
        });
